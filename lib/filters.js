  ...
  if (matches) {
    ...
    get_permit_page(item.id_permit, function(page) {
      ...
      if (page) {
       ...
       item.geoname = body('#exp_col_layer_1740 tr').eq(8).find('td').last().text()
       if (item.geoname.substr(item.geoname.length - 1) == '.') {
         item.geoname = item.geoname.substr(0, item.geoname.length - 1)
       }
       
       item.description = item.description.replace(regexp,'<a target="_blank" href="' + item.url_permit + '">' + item.id_permit + '</a> ('+item.geoname+')')
      }
       return callback(item)
