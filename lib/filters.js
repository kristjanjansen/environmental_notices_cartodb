...
exports.cad = function(item, callback, replace) {
...
exports.cad2 = function(item, callback) {
 if (item.type == 'Maa riigi omandisse jätmise teated') {
   ...
   if (matches) {
     ...
     each(matches)
       .on('item', function(next, desc, i) {
         var child = {}
         child.id = item.id.substr(0, item.id.length - 1) + (i + 1)
         child.type = 'Maa riigi omandisse jätmise teated #' + (i + 1)
         child.description = item.description.split('\n')[0] + '\n\n' + desc.substr(0, desc.length - 1)
...
exports.permit_water = function(item, callback) {
  ...
  //L.VV.LÄ-166343 
  //L.VV.HA-183003
  var matches = item.description.match(/(põhjave|puurkaev)/g)
  if (matches) {
    item.priority = 0
  }
