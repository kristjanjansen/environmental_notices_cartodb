    ...
    .on('item', function(next_row, body_row, i) {
      if (i % 3 == 0) {
        ...
        processRow(row, function(p_row) {
          var query = "INSERT INTO " + CONFIG.cartoTable + " (id, priority, date, type, description" + (p_row.the_geom ? ", the_geom" : "") + ") VALUES ('" + p_row.id + "', '" + p_row.priority + "', '" + p_row.date + "', '" + p_row.type + "', '" + p_row.description + "'" + (p_row.the_geom ? ", " + p_row.the_geom : "") + ")"
          cdb.query(query) 
        })
