...
each(urls)
   ...
   if (i == 0) { 
      ...
      request({url: url, encoding: 'binary'}, function (e, r, body) {
        processPage(body, function(rows) {
          console.log(moment().format(), 'Processing page', i);      
          setTimeout(next, 0)
        })
      });
    }
  ...
  .on('end', function() {
    console.log(moment().format(), 'End each');          
    callback()
  });
          ...
          if (p_row.childs.length < 2) {
            ...
            each(p_row.childs)
              .on('item', function(next, p_row, i) {
                var query = "INSERT INTO {table} (id, priority, date, type, description, description_short{geom_1}) VALUES ('{id}', '{priority}', '{date}', '{type}', '{description}', '{description_short}'{geom_2})"
                var params = {
                  table: CONFIG.cartoTable,
                  geom_1: p_row.the_geom ? ', the_geom' : '',
                  id: p_row.id,
                  ...
                  geom_2: p_row.the_geom ? ', ' + p_row.the_geom : '',
                }
                cdb.query(query, params, function(err, data) {
                  if (err) {
